...
use crate::scnr::{Token, TokenType, TokenType::*};
use crate::stmt::Stmt;
...
impl Parser {
    ...
    fn category_declaration(&mut self) -> Result<Stmt, String> {
        let name = self.consume(Identifier, "Expected name after 'category' keyword.")?;
        let supercat = if self.match_token(TokenType::Less) {
            self.consume(Identifier, "Expected supercat name after '<'.")?;
            Some(Expr::Variable {
                id: self.get_id(),
                name: self.previous(),
            })
        ...
        self.consume(LeftBrace, "Expected '{' before category body.")?;
        ...
        self.consume(RightBrace, "Expected '}' after category body.")?;
    ...
    fn primary(&mut self) -> Result<Expr, String> {
        ...
        match token.token_type {
            ...
            TokenType::Super => {
                ...
                let method =
                    self.consume(TokenType::Identifier, "Expected supercat method name.")?;
                result = Expr::Super {
                    id: self.get_id(),
                    keyword: token,
                    method,
