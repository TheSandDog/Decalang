    ...
    fn resolve_internal(&mut self, stmt: &Stmt) -> Result<(), String> {
        match stmt {
            ...
            Stmt::Cat {
                ...
                // Resolve supercat, if present
                if let Some(super_expr) = supercat {
                    ...
                    } = super_expr
                    {
                        if super_name.lexeme == name.lexeme {
                            return Err("A cat cannot inherit from itself".to_string());
                        }
                    }
            ...
            Stmt::Send { expression } => self.resolve_expr(expression)?,
        ...
        match expr {
            ...
            Expr::This { id: _, keyword } => {
                if self.current_function != FunctionType::Method {
                    return Err("Cannot use 'this' keyword outside of a cat".to_string());
                }
            ...
            Expr::Super {
                ...
                if self.current_function != FunctionType::Method {
                    return Err("Cannot use 'super' keyword outside of a cat".to_string());
                }
                if self.scopes.len() < 3
                    || !self.scopes[self.scopes.len() - 3].contains_key("super")
                {
                    return Err("cat has no supercat".to_string());
                }
                self.resolve_local(keyword, expr.get_id())
            }
            Expr::Unary {
