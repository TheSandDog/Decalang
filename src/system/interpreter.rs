        ...
        for stmt in stmts {
            match stmt {
                ...
                Stmt::Cat {
                    ...
                    // Insert the methods of the supercat into the methods of this cat
                    let supercat_value;
                    if let Some(supercat) = supercat {
                        let supercat = supercat.evaluate(self.environment.clone())?;
                        if let LiteralValue::Loxcat { .. } = supercat {
                            supercat_value = Some(Box::new(supercat));
                        } else {
                            return Err(format!(
                                "supercat must be a cat, not {}",
                                supercat.to_type()
                            ));
                        }
                    } else {
                    ...
                    if let Some(sc) = supercat_value.clone() {
                    ...
                    let klass = LiteralValue::Loxcat {
                        name: name.lexeme.clone(),
                        methods: methods_map,
                        supercat: supercat_value,
                    };

                    if !self.environment.assign_global(&name.lexeme, klass) {
                        return Err(format!("cat definition failed for {}", name.lexeme));
                    }
